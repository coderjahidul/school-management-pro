...
<thead>
	...
	<tr>
		...
		foreach ($exams as $key => $exam) {
			...
			?>
			<th><?php 
			// echo esc_html(stripslashes($exam->exam_title));
			esc_html_e('Maximum Marks', 'school-management');
			 ?></th>
		<?php
...
<tbody>
	...
	foreach ($subjects as $subject) {
		...
		// echo "<pre>";
		// print_r($new_subject_array);
		// echo "</pre>";
	foreach ($new_subject_array as $subject) {
	?>
		<tr>
			<td>
				<?php
...
<?php endif ?>
	...
	<tr>
		...
		foreach ($exams as $key => $exam) {
			...
			if ($admit_card) {
				$student_rank = WLSM_M_Staff_Examination::calculate_exam_ranks($school_id, $exam->ID, array(), $admit_card->ID , $student->note);
			}
		?>
			<td>
