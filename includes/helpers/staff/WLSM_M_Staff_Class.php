...
class WLSM_M_Staff_Class
{
	...
	public static function subject_list($type, $class_school_id)
	{
		global $wpdb;
		$all_subject_list = $wpdb->get_results( $wpdb->prepare("SELECT ID, code, label FROM {$wpdb->prefix}wlsm_subjects WHERE type = %s AND class_school_id = %d",
		$type,
		$class_school_id
		));
	...
	public static function optional_subject_code($admission_number)
	{
		global $wpdb;
		$optional_subject_list = $wpdb->get_results($wpdb->prepare("SELECT optional_subject_code FROM {$wpdb->prefix}wlsm_student_records WHERE admission_number = %d",
		$admission_number
		));
		return $optional_subject_list;
	}
	...
	{
		global $wpdb;
		$subject = $wpdb->get_row($wpdb->prepare('SELECT sj.ID, sj.label as subject_name, sj.code,  sj.subject_cat , sj.subject_group ,sj.parent_subject, sj.type,sj.religion, cs.class_id, c.label as class_label FROM ' . WLSM_SUBJECTS . ' as sj
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = sj.class_school_id
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			WHERE cs.school_id = %d AND sj.ID = %d', $school_id, $id));
		return $subject;
	...
	public static function get_class_subjects_in_new_exam($school_id, $class_id)
	{
		global $wpdb;
		// $exam_id =$wpdb->get_var( $wpdb->prepare(
		// 	"SELECT ID  FROM {$wpdb->prefix}wlsm_exams WHERE class_id =%d AND school_id = %d",
		// 	$class_id,
		// 	$school_id 
		// ));

			$subjects = $wpdb->get_results($wpdb->prepare('SELECT sj.ID, sj.label, sj.code, sj.type , sj.religion  FROM ' . WLSM_SUBJECTS . ' as sj
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = sj.class_school_id
		JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = cs.school_id
	...
	public static function get_class_subjects($school_id, $class_id)
	{
		...
		WHERE cs.school_id = %d AND cs.class_id = %d', $school_id, $class_id));
			...
			$subjects = $wpdb->get_results($wpdb->prepare('SELECT sj.ID, sj.label, sj.code, sj.type , sj.religion , exp.maximum_marks , exp.paper_date , exp.start_time , exp.end_time , exp.room_number FROM ' . WLSM_SUBJECTS . ' as sj
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = sj.class_school_id
			JOIN ' . WLSM_EXAM_PAPERS . ' as exp ON exp.paper_code = sj.code
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = cs.school_id
			WHERE cs.school_id = %d AND cs.class_id = %d AND exp.exam_id = %d', $school_id, $class_id, $exam_id));
	...
	public static function get_class_optional_subjects($school_id, $class_id) 
	{
		global $wpdb;

		// শুধুমাত্র objective subject গুলো আনার জন্য
		$subjects = $wpdb->get_results( 
			$wpdb->prepare(
				'SELECT sj.ID, sj.label, sj.code, sj.type, sj.religion 
				FROM ' . WLSM_SUBJECTS . ' as sj
				JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = sj.class_school_id
				JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
				JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = cs.school_id
				WHERE cs.school_id = %d AND cs.class_id = %d 
				AND sj.type = %s',
				$school_id, $class_id, 'objective'
			) 
		);
		return $subjects;
	}
