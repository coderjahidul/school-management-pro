...
class WLSM_M_Staff_Examination {
	...
	public static function get_exams_results_bulk_add() {
		return admin_url( 'admin.php?page=' . WLSM_MENU_STAFF_EXAM_RESULTS_ADD_RESULT );
	}
	...
	public static function fetch_exam_papers( $school_id, $exam_id ) {
		global $wpdb;
		$papers = $wpdb->get_results( $wpdb->prepare( 'SELECT DISTINCT ep.ID, ep.subject_label, ep.subject_type, ep.paper_code, ep.paper_order, ep.paper_date, ep.start_time, ep.end_time, ep.room_number, ep.maximum_marks, ep.religion FROM ' . WLSM_EXAM_PAPERS . ' as ep
	...
	// public static function fetch_class_subject()
	...
	public static function get_exam_admit_cards_for_bulk_print( $school_id, $exam_id, $class_id, $section_id = 0, $class_group ) {
		...
		$admit_cards = $wpdb->get_results( $wpdb->prepare( 'SELECT ac.ID, ac.exam_id,  ac.roll_number, sr.roll_number as student_class_roll_number , ac.student_record_id as student_id, sr.name, sr.phone, sr.enrollment_number, sr.photo_id, sr.email, sr.note ,c.label as class_label, se.label as section_label, ss.label as session_label FROM ' . WLSM_ADMIT_CARDS . ' as ac
			...
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = sr.session_id
			WHERE ex.school_id = %d AND ac.exam_id = %d AND cs.class_id = %d AND sr.note = %s GROUP BY ac.student_record_id ORDER BY ac.roll_number, sr.name', $school_id, $exam_id, $class_id, $class_group ) );
		...
		if ($section_id !== 0) {
			...
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = sr.session_id
			WHERE ex.school_id = %d AND ac.exam_id = %d AND cs.class_id = %d AND sr.section_id = %d AND sr.note = %s GROUP BY ac.student_record_id ORDER BY ac.roll_number, sr.name', $school_id, $exam_id, $class_id, $section_id ,$class_group) );
		} else {
	...
	public static function fetch_admit_card( $school_id, $id ) {
		global $wpdb;
		$admit_card = $wpdb->get_row( $wpdb->prepare( 'SELECT ac.ID, ac.exam_id, ac.roll_number, ac.student_record_id as student_id, sr.name, sr.phone, sr.enrollment_number, sr.admission_number, sr.photo_id, sr.email, sr.note, c.label as class_label, se.label as section_label, ss.label as session_label, sr.father_name FROM ' . WLSM_ADMIT_CARDS . ' as ac
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ac.exam_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
			JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
	...
	public static function fetch_student_admit_card( $school_id, $student_id, $id ) {
		global $wpdb;
		$admit_card = $wpdb->get_row( $wpdb->prepare( 'SELECT ac.ID, ac.exam_id, ac.roll_number, ac.student_record_id as student_id, sr.name, sr.phone, sr.enrollment_number, sr.admission_number, sr.roll_number as student_roll ,sr.photo_id, sr.email, sr.note, c.label as class_label, se.label as section_label, ss.label as session_label FROM ' . WLSM_ADMIT_CARDS . ' as ac
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ac.exam_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
			JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
	...
	public static function get_class_school_exams( $school_id, $class_school_id ) {
		global $wpdb;
		$exams = $wpdb->get_results( $wpdb->prepare( 'SELECT DISTINCT ex.ID, ex.label as exam_title, ex.show_rank, ex.show_remark, ex.psychomotor_analysis, ex.psychomotor, ex.start_date, ex.end_date, ex.results_obtained_marks , ex.enable_total_marks FROM ' . WLSM_EXAMS . ' as ex
		JOIN ' . WLSM_CLASS_SCHOOL_EXAM . ' as csex ON csex.exam_id = ex.ID
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = ex.school_id
	...
	public static function get_exam_results( $classGroup ,$school_id, $exam_id, $section_id, $order = 'ASC' ) {
		global $wpdb;
		$exam_results = $wpdb->get_results( $wpdb->prepare( 'SELECT ac.ID, ac.ID as admit_card_id, ep.exam_id, ac.roll_number, c.label as class_label, se.label as section_label, sr.name, sr.enrollment_number,sr.religion, sr.admission_number,sr.note,sr.session_id, sr.roll_number as student_class_roll_number, COALESCE(SUM(ep.maximum_marks), 0) as total_marks, COALESCE(SUM(er.obtained_marks), 0) as obtained_marks FROM ' . WLSM_EXAM_RESULTS . ' as er
			JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
			JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
			...
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			WHERE ex.school_id = %d AND ep.exam_id = %d AND sr.note = %s AND sr.section_id = %d GROUP BY ac.ID', $school_id, $exam_id , $classGroup, $section_id ) );
		return $exam_results;
	}
	// public static function count_student_rank($total_failde_subject){
	// 	global $wpdb;
	// 	$exam_results['total_failde_subject'] = $total_failde_subject;

	// 	return $exam_results;
	// }
	
	// for excel result print 
	public static function get_exam_results_for_excel( $school_id, $exam_id, $section_id ) {
				global $wpdb;
				if($section_id == 0){
					
					$exam_results = $wpdb->get_results( $wpdb->prepare(
						'SELECT ac.ID, ac.ID as admit_card_id, ep.exam_id, ep.ID as exam_paper_id ,  ac.roll_number, c.label as class_label, se.label as section_label, sr.name, sr.enrollment_number, sr.religion, sr.admission_number, sr.roll_number,ep.parent_subject, ep.subject_label, ep.paper_code , ep.maximum_marks as total_marks, COALESCE(er.obtained_marks, 0) as obtained_marks FROM ' . WLSM_EXAM_RESULTS . ' as er
						JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id
						JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
						JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
						JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
						...
						JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
						JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
						WHERE ex.school_id = %d AND ep.exam_id = %d ORDER BY admit_card_id ASC' ,
						$school_id,
						$exam_id
					) );
					...
					$exam_results = $wpdb->get_results( $wpdb->prepare(
						'SELECT ac.ID, ac.ID as admit_card_id, ep.exam_id, ac.roll_number, c.label as class_label, se.label as section_label, sr.name, sr.enrollment_number, sr.religion, sr.admission_number, sr.roll_number, ep.subject_label, ep.paper_code , ep.maximum_marks as total_marks, COALESCE(er.obtained_marks, 0) as obtained_marks FROM ' . WLSM_EXAM_RESULTS . ' as er
						JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id
						JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
						JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
						JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
						JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = %d
						JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
						JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
						WHERE sr.section_id = %d AND ex.school_id = %d AND ep.exam_id = %d ORDER BY admit_card_id ASC',
						$section_id,
						$section_id,
						$school_id,
						$exam_id
	...
	public static function get_exam_results_by_admit_card( $school_id, $admit_card_id ) {
		global $wpdb;
		$exam_results = $wpdb->get_results( $wpdb->prepare( 'SELECT er.exam_paper_id, er.ID, er.obtained_marks, er.remark, er.teacher_remark, er.school_remark, er.scale, er.answer_key, ac.exam_id , ac.student_record_id  FROM ' . WLSM_EXAM_RESULTS . ' as er
			JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
			JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
	...
	public static function get_exam_papers_by_admit_card( $school_id, $admit_card_id, $class_school_id = 0 ) {
		global $wpdb;
		if ( ! $class_school_id ) {
		...
		$exam_papers = $wpdb->get_results(
			$wpdb->prepare(
				'SELECT
					...
					esub.subject_group,
					esub.parent_subject,
					esub.ID as subject_id
				FROM
					' . WLSM_EXAM_PAPERS . ' as ep
				JOIN
					' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
				JOIN
					' . WLSM_SUBJECTS . ' as esub ON esub.code = ep.paper_code
				JOIN
					' . WLSM_ADMIT_CARDS . ' as ac ON ac.exam_id = ex.ID
				JOIN
					' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
				JOIN
					' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
				JOIN
					' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
				WHERE
					cs.school_id = %d
					AND ac.ID = %d
					AND esub.class_school_id = %d
					AND ep.religion = "common"
				GROUP BY
					ep.ID
				ORDER BY
					ep.paper_order ASC',
	...
	public static function get_religion_exam_papers_by_admit_card( $school_id, $admit_card_id, $class_school_id = 0 ) {
		global $wpdb;
		if ( ! $class_school_id ) {
			$class_school_id = $wpdb->get_var( $wpdb->prepare(
				"SELECT se.class_school_id FROM " . WLSM_ADMIT_CARDS . " as ac
	...
	public static function get_exam_papers_by_exam_id( $school_id, $exam_id ) {
		global $wpdb;
		$exam_papers = $wpdb->get_results( $wpdb->prepare( 'SELECT ep.ID, ep.subject_label, ep.subject_type,ep.religion, ep.paper_code, ep.maximum_marks FROM ' . WLSM_EXAM_PAPERS . ' as ep
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
			JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = ex.school_id
	...
	public static function calculate_exam_ranks( $school_id, $exam_id, $exam_results = array(), $admit_card_id = '', $student_group = '' ) {
		if ( empty( $exam_results ) && $school_id && $exam_id ) {
			global $wpdb;

			$query  = 'SELECT ac.ID, COALESCE(SUM(ep.maximum_marks), 0) as total_marks, COALESCE(SUM(er.obtained_marks), 0) as obtained_marks FROM ' . WLSM_EXAM_RESULTS . ' as er
				JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id
				JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
				JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
				JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
				WHERE ex.school_id = %d AND ep.exam_id = %d';
			$params = array( $school_id, $exam_id );
			...
			$exam_results = $wpdb->get_results( $wpdb->prepare( $query, $params ) );
		}
		...
		foreach ( $exam_results as $row ) {
			$id = isset( $row->ID ) ? $row->ID : $row->admit_card_id;
			$student_percentages[ $id ] = WLSM_Config::sanitize_percentage( $row->total_marks, $row->obtained_marks );
		}
