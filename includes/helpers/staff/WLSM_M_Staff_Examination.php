...
class WLSM_M_Staff_Examination {
	...
	public static function fetch_exam_papers( $school_id, $exam_id ) {
		global $wpdb;
		$papers = $wpdb->get_results( $wpdb->prepare( 'SELECT DISTINCT ep.ID, ep.parent_subject , ep.subject_label, ep.subject_type, ep.paper_code, ep.paper_order, ep.paper_date, ep.start_time, ep.end_time, ep.room_number, ep.maximum_marks, ep.religion FROM ' . WLSM_EXAM_PAPERS . ' as ep
	...
	public static function get_exam_admit_cards( $school_id, $exam_id ) {
		global $wpdb;
		$admit_cards = $wpdb->get_results( $wpdb->prepare( 'SELECT ac.ID, ac.exam_id,  ac.roll_number, sr.roll_number as student_class_roll_number,  ac.student_record_id as student_id, sr.name, sr.phone, sr.enrollment_number, sr.photo_id, sr.email, c.label as class_label, se.label as section_label, ss.label as session_label FROM ' . WLSM_ADMIT_CARDS . ' as ac
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ac.exam_id
	...
	public static function fetch_student_admit_card( $school_id, $student_id, $id ) {
		global $wpdb;
		$admit_card = $wpdb->get_row( $wpdb->prepare( 'SELECT ac.ID, ac.exam_id, ac.roll_number, ac.student_record_id as student_id, sr.name, sr.phone, sr.enrollment_number, sr.admission_number, sr.roll_number as student_roll ,sr.photo_id, sr.email, c.label as class_label, se.label as section_label, ss.label as session_label FROM ' . WLSM_ADMIT_CARDS . ' as ac
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ac.exam_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
			JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
	...
	public static function get_exam_results( $classGroup ,$school_id, $exam_id, $order = 'ASC' ) {
		global $wpdb;
		$exam_results = $wpdb->get_results( $wpdb->prepare( 'SELECT ac.ID, ac.ID as admit_card_id, ep.exam_id, ac.roll_number, c.label as class_label, se.label as section_label, sr.name, sr.enrollment_number,sr.religion, sr.admission_number,sr.note,sr.session_id, sr.roll_number, COALESCE(SUM(ep.maximum_marks), 0) as total_marks, COALESCE(SUM(er.obtained_marks), 0) as obtained_marks FROM ' . WLSM_EXAM_RESULTS . ' as er
			JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
			JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
			...
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
			JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			WHERE ex.school_id = %d AND ep.exam_id = %d AND sr.note = %s GROUP BY ac.ID', $school_id, $exam_id , $classGroup ) );
		return $exam_results;
	}
	...
	public static function get_exam_papers_by_admit_card( $school_id, $admit_card_id , $class_school_id  ) {
		global $wpdb;
	...
	public static function get_religion_exam_papers_by_admit_card( $school_id, $admit_card_id ,$class_school_id  ) {
		global $wpdb;
		$exam_papers = $wpdb->get_results(
			$wpdb->prepare(
				'SELECT
