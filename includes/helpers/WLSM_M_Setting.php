...
class WLSM_M_Setting {
	...
	// calculate cgpa based on letter grade
	public static function calculateGPA($letter_grade) {
		$grade_scale = array(
			'A+' => 5.00,
			'A'  => 4.00,
			'A-' => 3.50,
	...
	public static function optional_calculateGPA($optional_letter_grade){

		
		$grade_scale_optional = array(
			'A+' => 3.00,
			'A'  => 2.00,
			'A-' => 1.50,
			'B'  => 1.00,
		...
		$optional_upper_case_grade = strtoupper($optional_letter_grade);
		if(array_key_exists($optional_upper_case_grade, $grade_scale_optional)){
			return $grade_scale_optional[$optional_upper_case_grade];
		}else {
			return 'N/A';
		}
	}
    ...
    public static function calculate_gpa($subjects, $additional_subject = null) {
        $total_points = 0.00;
        $num_subjects = count($subjects);
        $lowest_point = PHP_FLOAT_MAX; // Initialize with a very high number

        // Calculate total grade points from main subjects
        foreach ($subjects as $marks) {
            $grade_point = self::get_grade_point($marks);
            $total_points += $grade_point;

            // Track the lowest grade point
            if ($grade_point < $lowest_point) {
                $lowest_point = $grade_point;
            }
        }

        // If additional subject is given, check if it improves GPA
        if ($additional_subject !== null) {
            $additional_gp = self::get_grade_point($additional_subject);
            if ($additional_gp > $lowest_point) {
                // Replace the lowest point with the additional subject's GP
                $total_points = $total_points - $lowest_point + $additional_gp;
            }
        }

        // Calculate GPA (without considering additional subject separately)
        $gpa = $total_points / $num_subjects;

        return round($gpa, 2); // Return GPA rounded to 2 decimal places
    }
}
