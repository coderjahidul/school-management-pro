...
// Prefer $admit_card_id when available — avoids fragile admission_number/section lookups.
function get_mark_by_paper_code($exam_id, $paper_code, $subject_type, $admission_number, $section_id, $admit_card_id = null){

	global $wpdb;

	$exam_paper_id = $wpdb->get_var($wpdb->prepare(
		"SELECT ID FROM {$wpdb->prefix}wlsm_exam_papers WHERE exam_id = %d AND paper_code = %s AND subject_type = %s",
		$exam_id,
		$paper_code,
		$subject_type
	));
	...
	if ( empty( $admit_card_id ) ) {
		$student_record_id = $wpdb->get_var( $wpdb->prepare(
			"SELECT ID FROM {$wpdb->prefix}wlsm_student_records WHERE admission_number = %s AND section_id = %d ",
			$admission_number,
			$section_id
		));

		if ( ! empty( $student_record_id ) ) {
			$admit_card_id = $wpdb->get_var( $wpdb->prepare(
				"SELECT ID FROM {$wpdb->prefix}wlsm_admit_cards WHERE exam_id = %d AND student_record_id = %d ",
				$exam_id,
				$student_record_id
...
function get_subject_maximum_mark_by_paper_code($exam_id, $paper_code, $subject_type){
	...
	$maximum_marks = $wpdb->get_var($wpdb->prepare(
		"SELECT maximum_marks FROM {$wpdb->prefix}wlsm_exam_papers WHERE exam_id = %d AND paper_code = %s AND subject_type = %s",
		$exam_id,
		$paper_code,
		$subject_type
	));
...
function get_section_id($enrollment_number, $admission_number, $session_id){
    ...
    $section_id = $wpdb->get_var( $wpdb->prepare(
        "SELECT section_id FROM {$wpdb->prefix}wlsm_student_records WHERE enrollment_number = %s AND admission_number = %s AND session_id = %d ",
        $enrollment_number,
        $admission_number,
        $session_id
    ));
