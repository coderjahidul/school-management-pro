...
<center class="excel_result_box" id="wlsm-print-result-excel">
	...
	// allowed option paper codes
	$allowed_optional_paper_codes = [
		"565", "566",
		"551", "552",
		"534", "538",
		"529", "530",
	...
	// allowed main paper codes
	$allowed_main_paper_codes = [
		136, 137, 138, 428, 429
	];
...
$admit_card_ids = $wpdb->get_col(
    $wpdb->prepare(
        "SELECT student_record_id 
         FROM {$wpdb->prefix}wlsm_admit_cards 
         WHERE exam_id = %d",
        $exam_id
    )
);

if (!empty($admit_card_ids)) {

    $placeholders = implode(',', array_fill(0, count($admit_card_ids), '%d'));

    $query = $wpdb->prepare(
        "SELECT *
         FROM {$wpdb->prefix}wlsm_student_records
         WHERE ID IN ($placeholders)
         AND section_id IN (" . implode(',', array_map('intval', $sections)) . ")
         AND note = %s
         ORDER BY roll_number ASC",
        array_merge($admit_card_ids, [$class_group])
    );

    $student_records = $wpdb->get_results($query);
	...
	// Get all subject labels related to the exam
	$class_subjects = $wpdb->get_results($wpdb->prepare(
		"SELECT code, subject_group FROM {$wpdb->prefix}wlsm_subjects WHERE class_school_id = %d",
		$class_school_id
	));
	$subject_paper_codes = []; // Initialize an array to store paper codes

	foreach ($class_subjects as $class_subject) {
		$subject_group = $class_subject->subject_group;
		$unserialize_group_data = unserialize($subject_group)['subject_group'];
		
		if (in_array($class_group, $unserialize_group_data)) {
			$subject_paper_codes[] = $class_subject->code; // Store paper codes in an array
		}
	}

	// Check if there are paper codes to process
	if (!empty($subject_paper_codes)) {
		// Prepare a placeholder string for the SQL query
		$placeholders = implode(',', array_fill(0, count($subject_paper_codes), '%d'));
		
		// Fetch all exam papers matching the paper codes
		$query = $wpdb->prepare(
			"SELECT subject_label, subject_type, paper_code, maximum_marks, religion 
			FROM {$wpdb->prefix}wlsm_exam_papers 
			WHERE paper_code IN ($placeholders) AND exam_id = %d",
			array_merge($subject_paper_codes, [$exam_id])
		);

		$exam_papers = $wpdb->get_results($query);
	} else {
		echo "No matching paper codes found.";
	}
		...
		foreach ($thirdfiltered as $subject) {
			foreach ($exam_papers as $subject) {
				...
				if ($subject_type == "subjective") { 
					...
					$bangla_second_maxmarks = $subject_maximum_marks;
					// print_r("Bangla Second: " . $bangla_second_maxmarks);
				} ?>
				...
				if ($subject_type == "subjective") { ?>
					...
					$english_second_maxmarks = $bangla_second_maxmarks;
					// print_r("English Second: " . $english_second_maxmarks);
				} ?>
	...
	<div class="table-container excel_result_box" style="width:100%;overflow-x: auto;" id="wlsm-print-result-excel">
		<table>
			<thead>
				<tr>
					<th colspan="100" style="text-align: center; font-size: 16px; font-weight: bold;">
						Class: <?php echo $class_label[0]->label; ?>
						& Group: <?php echo ucwords($class_group); ?> & Section:
						<?php
			...
			<tbody>
				<tr>
					...
					/* =========================
					STEP 1: ICT (154) check
					========================= */
					$ictSubjective = null;
					$ictMcq = null;
					$ictPractical = null;
					$ictHeaderPrinted = false;
					$banglaSecondMcq = false;

					foreach ($exam_papers as $subject) {
						if ($subject->paper_code == 154) {
							if ($subject->subject_type === 'subjective') {
								$ictSubjective = $subject;
							} elseif ($subject->subject_type === 'mcq') {
								$ictMcq = $subject;
							} elseif ($subject->subject_type === 'practical') {
								$ictPractical = $subject;
							}
						}
					}

					foreach ($exam_papers as $subject) {
						if ($subject->paper_code == 102 && $subject->subject_type === 'mcq') {
							$banglaSecondMcq = true;
						}
					}
					...
					foreach ($exam_papers as $subject) {
						...
						if ($class_label_id <= 5) {
							...
							if ($paper_code == 101) {
								...
								echo "<th colspan='4'>{$word[0]}</th>";
							} elseif (in_array($paper_code, $allowed_main_paper_codes)) {
							...
							/* =========================
							SPECIAL CASE: paper 154
							========================= */
							if ($paper_code == 154) {

								// subjective থাকলে mcq skip
								if ($ictSubjective !== null && $subject_type !== 'subjective') {
									continue;
								}

								// subjective না থাকলে শুধু mcq
								if ($ictSubjective === null && $subject_type !== 'mcq') {
									continue;
								}
							...
							if ($subject_type === 'subjective' || $subject_type === 'objective') {
								...
								if ($bangla_second_maxmarks != null || $english_second_maxmarks != null) {
									...
									} elseif (in_array($paper_code, $allowed_main_paper_codes) || $paper_code == 275) {
					...
					foreach ($exam_papers as $subject) {
						...
						if($class_label_id <= 5){
							...
							if($bangla_second_maxmarks != null || $english_second_maxmarks != null){
								...
								SPECIAL CASE: paper 154
								========================= */
								if ($paper_code == 154 && !$ictHeaderPrinted) {

									// Case 1: CQ + MCQ
									if ($ictSubjective && $ictMcq && !$ictPractical) {
										echo "<th>" . esc_html__("CQ", "school-management") . "</th>";
										echo "<th>" . esc_html__("MCQ", "school-management") . "</th>";
									}
									// Case 2: CQ + Practical
									elseif ($ictSubjective && !$ictMcq && $ictPractical) {
										echo "<th>" . esc_html__("CQ", "school-management") . "</th>";
										echo "<th>" . esc_html__("Practical", "school-management") . "</th>";
									}
									// Case 3: MCQ + Practical
									elseif (!$ictSubjective && $ictMcq && $ictPractical) {
										echo "<th>" . esc_html__("MCQ", "school-management") . "</th>";
										echo "<th>" . esc_html__("Practical", "school-management") . "</th>";
									}
									...
									// Common columns
									echo "<th>" . esc_html__("Total", "school-management") . "</th>";
									echo "<th>" . esc_html__("LG", "school-management") . "</th>";
									echo "<th>" . esc_html__("GP", "school-management") . "</th>";

									// mark as printed
									$ictHeaderPrinted = true;
								...
								if ($subject_type === 'subjective' || $subject_type === 'objective') {
									...
									<?php } elseif ($paper_code == 108) { ?>
									<!-- Nathing -->
									<?php } elseif ($paper_code == 154) { ?>
									<!-- Nathing -->
									<?php } elseif (in_array($paper_code, $allowed_main_paper_codes) || $paper_code == 275) { ?>
										<th>
								...
								if ($subject_type === 'subjective' || $subject_type === 'objective') {
									if ($paper_code == 101 || $paper_code == 401) { ?>
										<!-- Bangla -->
										<th>
											<?php esc_html_e("CQ", "school-management") ?>
										</th>
									...
									<?php } elseif ($paper_code == 102) { ?>
										<!-- Nathing -->
									<?php } elseif ($paper_code == 107 || $paper_code == 407) { ?>
										<!-- English -->
										<th>
											<?php esc_html_e("CQ", "school-management") ?>
										</th>
									...
									<?php } elseif ($paper_code == 154) { ?>
									<!-- Nathing -->
									<?php } elseif (in_array($paper_code, $allowed_main_paper_codes)) { ?>
										<th>
											<?php esc_html_e("CQ", "school-management") ?>
										</th>
										<th>
				...
				foreach ($students as $student): ?>
					...
					<tr>
									...
									if ($paper_code == 101) { ?>
										...
										if ($subject_type == "subjective") { ?>
											...
											<td>
												...
												$minimam_cq_fash_marks = floor($bangla_subject_maxmarks / 3);
	
												$divide_bangla_mark = $total_bangla_mark / 1;
												// total marks to be divided by 100 to get percentage 
												$bangla_maximam_marks = $bangla_first_cq_maxmarks;
												...
												if ($bangla_first_cq_marks >= 1) {
													if ($bangla_total_cq_marks >= $minimam_cq_fash_marks) {
														...
														$main_subject[] = floor($divide_bangla_mark);
										...
										if ($subject_type == "subjective") { ?>
											...
											<td>
												...
												if ($english_first_cq_marks >= 1) {
													...
													$main_subject[] = floor($divide_english_mark);
											...
											<td>
												<?php
												if ($english_first_cq_marks >= 1) {
													$subject_gpa = number_format(WLSM_M_Setting::calculateGPA($letter_grade_english), 2);
													echo $subject_gpa;
													$gpa_count += $subject_gpa;
													$main_subject_count += 1;
												} else {
										...
										if ($subject_type == "subjective") { ?>
											...
											<td>
												...
												if ($other_subject_cq_marks >= 1) {
													...
													if ($other_subject_cq_marks >= $minimam_other_subject_cq_fash_mark) {
														...
														$main_subject[] = floor($other_subject_marks);
											...
											<td>
												...
												if ($other_subject_cq_marks >= 1) {
													if ($other_subject_cq_marks >= $minimam_other_subject_cq_fash_mark) {
														$subject_gpa = number_format(WLSM_M_Setting::calculateGPA($other_subject_letter_grade), 2);
														echo $subject_gpa;
														$gpa_count += $subject_gpa;
														$main_subject_count += 1;
													} else {
									...
									foreach ($exam_papers as $subject) {
										...
										if ($paper_code == 101) { ?>
											...
											if ($subject_type == "subjective") { ?>
												...
												<?php if($banglaSecondMcq != true){ // if Bangla second mcq not exists
													?>
													<td>
														<!-- Nothing -->
													</td>
													...
													<td>
														...
														$divide_bangla_subject_maxmarks = floor($bangla_subject_maxmarks / 2);
														// total marks to be divided by 100 to get percentage 
														$bangla_maximam_marks = $divide_bangla_subject_maxmarks;
														$subje_maximam_marks = $bangla_maximam_marks;
														$grades_percentage = 100 / $subje_maximam_marks;
														$divide_bangla_mark *= $grades_percentage;

														if($class_label_id >=6 && $class_label_id <= 8){
															$letter_grade_bangla = esc_html(WLSM_Helper::calculate_grade($marks_grades, $divide_bangla_mark));
															$main_subject[] = floor($divide_bangla_mark);
															echo $letter_grade_bangla;
														}else{
											...
											if ($subject_type == "subjective") { ?>
												...
												<td>
													...
													// divide english maximum mark by 2 to get percentage
													$divide_english_subject_maxmarks = floor($english_subject_maxmarks / 2);
													// total marks to be divided by 100 to get percentage 
													$english_maximam_marks = $divide_english_subject_maxmarks;
													$grades_percentage = 100 / $english_maximam_marks;
													$divide_english_mark *= $grades_percentage;
										...
										<?php
											...
											if ($subject_type == "mcq") { ?>
												<td>
													...
													// echo esc_html("MCQ MAX: " . $group_subject_mcq_maxmarks);
											...
											if ($subject_type == "practical") { ?>
												...
												<td>
													...
													if ($group_subject_cq_marks >= 1 || $group_subject_mcq_marks >= 1 || $group_subject_practical_marks >= 1) {
														...
														if($class_label_id >=6 && $class_label_id <= 8){
															...
															$main_subject[] = floor($group_subject_marks);
												...
												<td>
													...
													if ($group_subject_cq_marks >= 1 || $group_subject_mcq_marks >= 1 || $group_subject_practical_marks >= 1) {
														if($class_label_id >=6 && $class_label_id <= 8){
															$subject_gpa = number_format(WLSM_M_Setting::calculateGPA($group_subject_letter_grade), 2);
															echo $subject_gpa;
															$gpa_count += $subject_gpa;
															$main_subject_count += 1;
														}else{
										...
										} elseif (in_array($paper_code, $allowed_optional_paper_codes)) {
												...
												<td>
													...
													if ($objective_cq_marks >= 1 || $objective_mcq_marks >= 1 || $objective_practical_marks >= 1) {
														...
														if($class_label_id >=6 && $class_label_id <= 8){
															...
															$additional_subject = $objective_marks;
											...
											<?php }
										} elseif ($paper_code == 154) {
											/* ================= THEORY MARKS ================= */
											// CQ / Subjective
											if ($subject_type == "subjective") { ?>
												<td>
													<?php
													$cq_marks = $get_obtained_marks;
													$cq_maxmarks = $subject_maximum_marks;
													$has_cq = true;

													if ($cq_marks >= 1) {
														echo esc_html($cq_marks);
													}
													?>
												</td>
											<?php }
											...
											if ($subject_type == "mcq") { ?>
												<td>
													<?php
													$mcq_marks = $get_obtained_marks;
													$mcq_maxmarks = $subject_maximum_marks;
													$has_mcq = true;

													if ($mcq_marks >= 1) {
														echo esc_html($mcq_marks);
													}
													?>
												</td>
												<?php 
												...
												?>
													...
													<td>
														...
														// CQ + MCQ
														if ($has_cq && $has_mcq) {
															$total_marks = $cq_marks + $mcq_marks;
															$total_maxmarks = $cq_maxmarks + $mcq_maxmarks;

															$min_cq   = floor($cq_maxmarks / 3);
															$min_mcq  = floor($mcq_maxmarks / 3);

															$pass = ($cq_marks >= $min_cq && $mcq_marks >= $min_mcq);
														}
														

														echo $total_marks;
														$other_subject_total_marks += $total_marks;
														?>
													</td>
													...
													<td>
														<?php
														if($class_label_id >=6 && $class_label_id <= 8){
															$percentage = ($total_marks / $total_maxmarks) * 100;
															$letter_grade = esc_html(
																WLSM_Helper::calculate_grade($marks_grades, $percentage)
															);
															echo $letter_grade;
															$main_subject[] = floor($percentage);
														}else{
															if ($pass) {
																$percentage = ($total_marks / $total_maxmarks) * 100;
																$letter_grade = esc_html(
													...
													<td>
														...
														if($class_label_id >=6 && $class_label_id <= 8){
															$gpa = number_format(
																WLSM_M_Setting::calculateGPA($letter_grade),
																2
															);
														}else{
															if ($pass) {
											...
											if ($subject_type == "practical") { ?>
												<td>
													<?php
													$practical_marks = $get_obtained_marks;
													$practical_maxmarks = $subject_maximum_marks;
													$has_practical = true;

													if ($practical_marks >= 1) {
														echo esc_html($practical_marks);
													}
													?>
												</td>
												<?php 
													if(($has_cq && !$has_mcq) || ($has_mcq && !$has_cq)){
												?>
												<!-- ================= TOTAL ================= -->
												<td>
													<?php
													// MCQ + Practical
													if ($has_mcq && $has_practical) {
														$total_marks = $mcq_marks + $practical_marks;
														$total_maxmarks = $mcq_maxmarks + $practical_maxmarks;

														$min_mcq  = floor($mcq_maxmarks / 3);
														$min_prac = floor($practical_maxmarks / 3);

														$pass = ($mcq_marks >= $min_mcq && $practical_marks >= $min_prac);
													}

													// CQ + Practical
													elseif ($has_cq && $has_practical) {
														$total_marks = $cq_marks + $practical_marks;
														$total_maxmarks = $cq_maxmarks + $practical_maxmarks;

														$min_cq   = floor($cq_maxmarks / 3);
														$min_prac = floor($practical_maxmarks / 3);

														$pass = ($cq_marks >= $min_cq && $practical_marks >= $min_prac);
													}
								...
								foreach ($thirdfilteredArray as $subject) {
									foreach ($exam_papers as $subject) {
										...
										if ($paper_code == 101) { ?>
											...
											<?php } ?>
											<?php
										} elseif ($paper_code == 107 || $paper_code == 407) {
											?>
											<!-- English first cq-->
											<?php
											if ($subject_type == "subjective") { ?>
										...
										<?php
											...
											if ($subject_type == "practical") { ?>
												...
												<td>
													...
													if ($objective_cq_marks >= 1 || $objective_mcq_marks >= 1 || $objective_practical_marks >= 1) {
														if($class_label_id >=6 && $class_label_id <= 8){
															$optional_gpa = number_format(WLSM_M_Setting::optional_calculateGPA($objective_letter_grade), 2);
															echo $optional_gpa;
															$additional_gpa_count += $optional_gpa;
														}else{
						...
						<td>
							...
							if($without_objective_total_mark_percentage <= 100 && $objective_total_marks >= 40 && $optional_gpa != "0.00"){
							...
							if ($total_failde_subject == 0) {
								$without_objective_letter_grade = esc_html(WLSM_M_Setting::calcuateGPAToLetterGrade($_gpa_result));
						...
						<td>
							<?php
								...
								if ($total_failde_subject == 0) {
									$_gpa = number_format(WLSM_M_Setting::calculatePreciseGPA($total_mark_percentage), 2);
									$gap_and_additional_gpa = $gpa_count + $additional_gpa_count;
									$gro_gpa = number_format(($gap_and_additional_gpa / $main_subject_count), 2);
									if($gro_gpa > 5.00){
										$gro_gpa = number_format(5.00, 2);
									}
							...
							if ($total_failde_subject == 0) {
								$_final_grade = esc_html(WLSM_M_Setting::calcuateGPAToLetterGrade($gro_gpa));
								echo $_final_grade;
							} else {
								echo "F";
							}
...
<?php if (!isset($is_download) || !$is_download): ?>
<div class="wlsm-container d-flex mb-2">
	<div class="col-md-12 wlsm-text-center">
		...
		<button type="button" class="<?php echo esc_attr($print_button_classes); ?> ml-2" id="wlsm-download-result-excel-btn" data-exam="<?php echo esc_attr($exam_id); ?>" data-class="<?php echo esc_attr($class_id); ?>" data-section="<?php echo esc_attr($section_id); ?>" data-group="<?php echo esc_attr($class_group); ?>">
			<?php esc_html_e('Download Excel', 'school-management'); ?>
		</button>
	</div>
</div>
<?php endif; ?>