...
class WLSM_Staff_Class
{
	...
	public static function save_subject()
	{
		...
		try {
			...
			$label     = isset($_POST['label']) ? sanitize_text_field($_POST['label']) : '';
			$subject_cat   = isset($_POST['subject_cat']) ? sanitize_text_field($_POST['subject_cat']) : '';
			$parent_subject   = isset($_POST['parent_subject']) ? sanitize_text_field($_POST['parent_subject']) : '';
			$code      = isset($_POST['code']) ? sanitize_text_field($_POST['code']) : '';
			$type      = isset($_POST['type']) ? sanitize_text_field($_POST['type']) : '';
			$sub_religions = isset($_POST['sub_religion']) ? sanitize_text_field($_POST['sub_religion']) : '';
			...
			$subject_groups = isset($_POST['calss_group']) && is_array($_POST['calss_group']) ? $_POST['calss_group'] : array();

		

			$subject_group_to_insert = serialize(
				array(
			...
			try {
				...
				foreach ($class_ids as $class_id){
					...
					$data = array(
						...
						'subject_group'  => $subject_group_to_insert,
						'code'            => $code,
						'type'            => $type,
						'class_school_id' => $class_school_id,
						'religion' => $sub_religions,
					);
				...
				wp_send_json_success(array('message' => $message, 'reset' => $reset , 'subject_group'=> $subject_groups));
			} catch (Exception $exception) {
				$wpdb->query('ROLLBACK;');
				wp_send_json_error($exception->getMessage());
			}
	...
	public static function fetch_staff_timetable() {
		...
		if (count($filter_rows_limit)) {
			foreach ($filter_rows_limit as $row) {
				...
				$days = WLSM_Helper::meeting_weekly_days();

				if ($row->day) {
					foreach ($days as $key => $value) {			
						if ($key === intval($row->day)) {
							$day = $value;
						}
					}
				}
				...
				$data[] = array(
					...
					esc_html(WLSM_M_Class::get_label_text($day)),
					esc_html(WLSM_M_Class::get_label_text($row->day)),
