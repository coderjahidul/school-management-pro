...
class WLSM_Staff_General
{
	...
	public static function get_all_section_student(){
		global $wpdb;
		$class_id   = isset($_POST['class_id']) ? absint($_POST['class_id']) : 0;
		$section_id = isset($_POST['section_id']) ? absint($_POST['section_id']) : 0; 
		$session_id = $current_user['session']['ID'];
			...
			$students = $wpdb->get_results(
				$wpdb->prepare(
					'SELECT sr.ID, sr.name, sr.enrollment_number FROM ' . WLSM_STUDENT_RECORDS . ' as sr 
					WHERE  sr.section_id = %d' ,  $section_id )
		 );
	...
	public static function get_class_optional_subjects()
	{
		$current_user = WLSM_M_Role::can(array('manage_timetable', 'view_timetable', 'manage_live_classes'));
		...
		try {
			...
			if (!wp_verify_nonce($_POST['nonce'], 'get-class-optional-subjects')) {
				die();
			}
			...
			$subjects = WLSM_M_Staff_Class::get_class_optional_subjects($school_id, $class_id);
	...
	public static function get_class_subjects_exam() {
		...
		try {
			...
			$subjects = WLSM_M_Staff_Class::get_class_subjects_in_new_exam($school_id, $class_id);
	...
	public static function save_student($current_user)
	{
		...
		try {
			...
			$optional_subject_code    = isset($_POST['optional_subject_code']) ? sanitize_text_field($_POST['optional_subject_code']) : '';
		...
		if (count($errors) < 1) {
			try {
				...
				$student_record_data = array(
					...
					'room_id'           => $room_id,
					'is_active'         => $is_active,
					'optional_subject_code' => $optional_subject_code,
				);
...
?>
	...
	public static function print_testimonials_cards()
	{
		global $wpdb;
		$current_user = WLSM_M_Role::can('manage_students');
		...
		// $class_id    = isset($_POST['class_id']) ? absint($_POST['class_id']) : 0;
		$section_id  = isset($_POST['section_id']) ? absint($_POST['section_id']) : 0;
		$section_student_id = isset($_POST['section_student']) ? absint($_POST['section_student']) : 0;
		$kromik_number = isset($_POST['kromik_number'])? $_POST['kromik_number']: "";
		$today_date = isset($_POST['today_date'])? $_POST['today_date']: "";
		$student_name = isset($_POST['student_name'])? $_POST['student_name']: "";
		$father_name = isset($_POST['father_name'])? $_POST['father_name']: "";
		$mother_name = isset($_POST['mother_name'])? $_POST['mother_name']: "";
		$board_roll = isset($_POST['board_roll'])? $_POST['board_roll']: "";
		$registration_number = isset($_POST['registration_number'])? $_POST['registration_number']: "";
		$session = isset($_POST['session'])? $_POST['session']: "";
		$board = isset($_POST['board'])? $_POST['board']: "";
		$passing_date = isset($_POST['passing_date'])? $_POST['passing_date']: "";
		$cgpa = isset($_POST['cgpa'])? $_POST['cgpa']: "";
		$second_school_name = isset($_POST['second_school_name'])? $_POST['second_school_name']: "";
		$school_name = isset($_POST['school_name'])? $_POST['school_name']: "";
		$school_location = isset($_POST['school_location'])? $_POST['school_location']: "";
		$school_established = isset($_POST['school_established'])? $_POST['school_established']: "";
		$school_id = isset($_POST['school_id'])? $_POST['school_id']: 0;
		$testimonials_name = isset($_POST['testimonials_name'])? $_POST['testimonials_name']: "";


	 	$students = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . WLSM_STUDENT_RECORDS . ' 
			WHERE ID = %d AND section_id = %d' , $section_student_id, $section_id ) );
		
	

			ob_start();

		require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/print/testimonials.php';
			$html = ob_get_clean();
	...
	public static function fetch_student_data_testimonials_cards()
	{
		global $wpdb;
		$current_user = WLSM_M_Role::can('manage_students');
	...
	public static function print_admit_cards() {
		...
		$class_group     = isset($_POST['class_group']) ? $_POST['class_group'] : '';
		$only_active = isset($_POST['only_active']) ? (bool) ($_POST['only_active']) : 1;
		...
		if (count($errors) < 1) {
			$admit_cards = WLSM_M_Staff_Examination::get_exam_admit_cards_for_bulk_print( $school_id, $exam_id, $class_id, $section_id, $class_group );
			...
			wp_send_json_success(array('html' => $html, 'json' => $json , 'class_group'=>$class_group, 'class_id'=>$class_id , 'school_id'=>$school_id));
		}

		wp_send_json_error($errors);
	}

	public static function new_testimonials_eng()
	{
		global $wpdb;
		$current_user = WLSM_M_Role::can('manage_students');
		...
		require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/print/testimonials_eng.php';
			$html = ob_get_clean();
	...
	public static function save_staff($current_user, $role)
	{
		...
		try {
			...
			$email         = isset($_POST['email']) ? sanitize_text_field($_POST['email']) : '';
			$blood_group   = isset($_POST['blood_group']) ? sanitize_text_field($_POST['blood_group']) : '';
			$mpo_date      = isset($_POST['mpo_date']) ? DateTime::createFromFormat(WLSM_Config::date_format(), sanitize_text_field($_POST['mpo_date'])) : NULL;
			$photo         = (isset($_FILES['photo']) && is_array($_FILES['photo'])) ? $_FILES['photo'] : NULL;
			$phone         = isset($_POST['phone']) ? sanitize_text_field($_POST['phone']) : '';
			...
			if(!empty($mpo_date)) {
				$mpo_date = $mpo_date->format('Y-m-d');
			} else {
				$mpo_date = NULL;
			}
		...
		if (count($errors) < 1) {
			try {
				...
				if ($admin_id) {
					$admin_data['photo_id'] = $admin->photo_id;
				}
	...
	public static function manage_promotion()
	{
		...
		if (count($errors) < 1) {
			try {
				...
				if (count($students)) {
					$from_optional_subjects = WLSM_M_Staff_Class::get_class_optional_subjects($school_id, $from_class_id);
					$to_optional_subjects   = WLSM_M_Staff_Class::get_class_optional_subjects($school_id, $to_class_id);

					$from_optional_subjects_json = array();
					foreach ($from_optional_subjects as $subject) {
						$from_optional_subjects_json[] = array(
							'code'  => $subject->code,
							'label' => WLSM_M_Class::get_label_text($subject->label),
						);
					}

					$to_optional_subjects_json = array();
					foreach ($to_optional_subjects as $subject) {
						$to_optional_subjects_json[] = array(
							'code'  => $subject->code,
							'label' => WLSM_M_Class::get_label_text($subject->label),
						);
					...
					$student_group_options = array(
						'common'     => esc_html__('Common', 'school-management'),
						'science'    => esc_html__('Science', 'school-management'),
						'commerce'   => esc_html__('Commerce', 'school-management'),
						'humanity'   => esc_html__('Humanity', 'school-management'),
						'vocational' => esc_html__('Vocational', 'school-management'),
					);
					...
					<div class="wlsm-form-section">
						...
						<div class="table-responsive w-100">
							<table class="table table-bordered wlsm-students-to-promote-table"
								data-from-class-id="<?php echo esc_attr($from_class_id); ?>"
								data-to-class-id="<?php echo esc_attr($to_class_id); ?>"
								data-from-optional-subjects="<?php echo esc_attr(wp_json_encode($from_optional_subjects_json)); ?>"
								data-to-optional-subjects="<?php echo esc_attr(wp_json_encode($to_optional_subjects_json)); ?>">
								<thead>
									<tr class="bg-primary text-white">
										...
										<th><?php esc_html_e('Optional Subject', 'school-management'); ?></th>
										<th><?php esc_html_e('Student Group', 'school-management'); ?></th>
										<th><?php esc_html_e('Student Type', 'school-management'); ?></th>
										<th><?php esc_html_e('Options', 'school-management'); ?></th>
									</tr>
								</thead>
								<tbody>
									<?php foreach ($students as $row) {
										$row_student_type = !empty($row->student_type) ? $row->student_type : 'regular';
										?>
										<tr>
											<td>
											...
											<td>
												<input type="text"
													name="roll_number[<?php echo esc_attr($row->ID); ?>]"
													class="form-control"
													value="<?php echo esc_attr($row->roll_number); ?>"
													placeholder="<?php esc_attr_e('Roll Number', 'school-management'); ?>">
											</td>
											<td>
												<select name="optional_subject_code[<?php echo esc_attr($row->ID); ?>]"
													class="form-control wlsm-promote-optional-subject"
													title="<?php esc_attr_e('Select Subject', 'school-management'); ?>">
													<option value=""><?php esc_html_e('Select Subject', 'school-management'); ?></option>
													<?php foreach ($to_optional_subjects as $subject) { ?>
														<option value="<?php echo esc_attr($subject->code); ?>" <?php selected($subject->code, $row->optional_subject_code, true); ?>>
											...
											<td>
												<select name="note[<?php echo esc_attr($row->ID); ?>]" class="form-control">
													<option value=""><?php esc_html_e('Select Group', 'school-management'); ?></option>
													<?php foreach ($student_group_options as $group_key => $group_label) { ?>
														<option value="<?php echo esc_attr($group_key); ?>" <?php selected($group_key, $row->note, true); ?>>
											...
											<td>
												<select name="student_type[<?php echo esc_attr($row->ID); ?>]" class="form-control">
													<?php foreach ($student_type_list as $type_key => $type_label) { ?>
														<option value="<?php echo esc_attr($type_key); ?>" <?php selected($type_key, $row_student_type, true); ?>>
											...
											<td>
												<select name="new_session_class[<?php echo esc_attr($row->ID); ?>]" class="wlsm-promote-new-class">
	...
	public static function promote_student()
	{
		...
		try {
			...
			$roll_numbers     = (isset($_POST['roll_number']) && is_array($_POST['roll_number'])) ? $_POST['roll_number'] : array();
			$optional_codes   = (isset($_POST['optional_subject_code']) && is_array($_POST['optional_subject_code'])) ? $_POST['optional_subject_code'] : array();
			$student_notes    = (isset($_POST['note']) && is_array($_POST['note'])) ? $_POST['note'] : array();
			$student_types    = (isset($_POST['student_type']) && is_array($_POST['student_type'])) ? $_POST['student_type'] : array();

			$valid_group_keys = array('common', 'science', 'commerce', 'humanity', 'vocational');
			$valid_type_keys  = array_keys(WLSM_Helper::student_type());

			$from_optional_subjects = array();
			$to_optional_subjects   = array();

			if (!empty($from_class->ID)) {
				$from_optional_subjects = WLSM_M_Staff_Class::subject_list('objective', $from_class->ID);
			}
			if (!empty($to_class->ID)) {
				$to_optional_subjects = WLSM_M_Staff_Class::subject_list('objective', $to_class->ID);
			}

			$from_optional_codes = array_filter(array_map(function ($subject) {
				return $subject->code;
			}, $from_optional_subjects));
			$to_optional_codes = array_filter(array_map(function ($subject) {
				return $subject->code;
			}, $to_optional_subjects));
		...
		try {
			...
			foreach ($student_ids_keys as $student_id) {
				if (isset($students[$student_id])) {
					...
					$new_roll_number = isset($roll_numbers[$student_id]) ? sanitize_text_field($roll_numbers[$student_id]) : '';
					if ('' === $new_roll_number) {
						$new_roll_number = $student->roll_number;
					}

					$optional_subject_code = isset($optional_codes[$student_id]) ? sanitize_text_field($optional_codes[$student_id]) : '';
					if ('' !== $optional_subject_code) {
						$valid_optional_codes = ($from_class_id == $new_class_ids[$student_id]) ? $from_optional_codes : $to_optional_codes;
						if (!in_array($optional_subject_code, $valid_optional_codes, true)) {
							throw new Exception(esc_html__('Please select a valid optional subject.', 'school-management'));
						}
					}

					$new_note = isset($student_notes[$student_id]) ? sanitize_text_field($student_notes[$student_id]) : '';
					if ('' !== $new_note && !in_array($new_note, $valid_group_keys, true)) {
						throw new Exception(esc_html__('Please select a valid student group.', 'school-management'));
					}
					if ('' === $new_note && !isset($student_notes[$student_id])) {
						$new_note = $student->note;
					}

					$new_student_type = isset($student_types[$student_id]) ? sanitize_text_field($student_types[$student_id]) : '';
					if ('' === $new_student_type) {
						$new_student_type = !empty($student->student_type) ? $student->student_type : 'regular';
					}
					if (!in_array($new_student_type, $valid_type_keys, true)) {
						throw new Exception(esc_html__('Please select a valid student type.', 'school-management'));
					}
