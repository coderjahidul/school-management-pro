...
class WLSM_Staff_Examination {
	...
	public static function get_student_record_id() {
		...
		foreach($data as $key => $studentInfo){
			...
			$studentData =$wpdb->get_results( $var = $wpdb->prepare(
				...
				WHERE sd.ID = %d AND sd.note = %s ORDER BY TRIM(sd.roll_number) DESC",
				$studentInfo->student_record_id , $class_group
			));
	...
	public static function wlsm_add_bulk_result() {
		...
		global $wpdb;
		$examPaperid = isset( $_POST['exam_paper_id'] ) ? sanitize_text_field( $_POST['exam_paper_id'] ) : 0;
	...
	public static function bulk_print_result_excel(){
		...
		if (count($errors) < 1) {
			// $results = WLSM_M_Staff_Examination::get_exam_results_for_excel( $school_id, $exam_id, $section_id );
			// $studentNameResult = WLSM_M_Staff_Examination::get_exam_results( $school_id, $exam_id );
			// $exam = WLSM_M_Staff_Examination::fetch_exam( $school_id, $exam_id );
			...
			$bulk_print = true;
			ob_start();
			require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/print/result_print_excel.php';
			$html = ob_get_clean();
