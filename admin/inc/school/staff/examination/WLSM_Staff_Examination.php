...
class WLSM_Staff_Examination {
	...
	public static function save_exam() {
		...
		try {
			...
			$sub_religion   = ( isset( $_POST['sub_religion'] ) && is_array( $_POST['sub_religion'] ) ) ? $_POST['sub_religion'] : array();
				...
				if ( ! count( $paper_code ) ) {
					...
					// }
						...
						foreach ( $paper_code as $key => $value ) {
							...
							$sub_religion[ $key ]   = sanitize_text_field( $sub_religion[ $key ] );
			...
			try {
				...
				foreach ( $paper_code as $key => $value ) {
					...
					if ( $is_insert ) {
						...
						$exam_paper_exist = $wpdb->get_row( $wpdb->prepare( 'SELECT ep.ID FROM ' . WLSM_EXAM_PAPERS . ' as ep WHERE ep.exam_id = %d AND ep.paper_code = %s AND ep.subject_type = %s', $exam_id, $value, $subject_type[$key] ) );
	...
	// get class based exam 
	public static function fetch_exams_class_based() {

		$schoolId = isset( $_POST['school_id'] ) ? sanitize_text_field( $_POST['school_id'] ) : 0;
		$classId = isset( $_POST['classId'] ) ? sanitize_text_field( $_POST['classId'] ) : 0;

		global $wpdb;
		$data =$wpdb->get_results( $wpdb->prepare(
			'SELECT ex.ID, ex.label AS exam_title, ex.exam_center, ex.start_date, ex.end_date, ex.is_active, ex.admit_cards_published, ex.time_table_published, ex.results_published
			FROM ' . WLSM_EXAMS . ' AS ex
			WHERE ex.school_id = %d AND ex.class_id = %d',
			$schoolId, $classId
		));
		// $output = array(
		// 	'data'            => $data,
	...
	// get class based section 
	public static function get_class_based_section() {
		...
		$data =$wpdb->get_var( $wpdb->prepare(
			'SELECT  ID   FROM ' . WLSM_CLASS_SCHOOL . ' 
			WHERE school_id = %d AND class_id = %d',
			$schoolId, $classId
		));

		$section =$wpdb->get_results( $wpdb->prepare(
			'SELECT  *   FROM ' . WLSM_SECTIONS . ' 
			WHERE class_school_id = %d',
			$data
		));
	...
	// get exam papers based on exam 
	public static function fetch_exam_papers_on_exam() {
		...
		$examId = isset( $_POST['examId'] ) ? sanitize_text_field( $_POST['examId'] ) : 0;
		$class_id = isset( $_POST['class_id'] ) ? sanitize_text_field( $_POST['class_id'] ) : 0;
		$section_id = isset( $_POST['section_id'] ) ? sanitize_text_field( $_POST['section_id'] ) : 0;
		$class_group = isset( $_POST['class_group'] ) ? sanitize_text_field( $_POST['class_group'] ) : '';
		
		global $wpdb;
		$exam_papers =$wpdb->get_results( $wpdb->prepare(
			'SELECT ex.ID, ex.subject_label AS exam_papers_name, ex.paper_code
			FROM ' . WLSM_EXAM_PAPERS . ' AS ex 
			WHERE ex.exam_id = %d',
			$examId
		));


		// get class school id by class id 
		$class_school_id =$wpdb->get_var( $wpdb->prepare( 'SELECT ID FROM ' . WLSM_CLASS_SCHOOL . ' WHERE class_id = %d AND school_id = %d', $class_id , $school_id  ));




		$filtered_all_exam_papers = [];

		foreach ($exam_papers as $key => $exam_paper) {
			$subject = $wpdb->get_results($wpdb->prepare(
				'SELECT *
				FROM ' . WLSM_SUBJECTS . '
				WHERE code = %d AND class_school_id = %d',
				$exam_paper->paper_code, $class_school_id
			));
		
			$unsirData = $subject[0]->subject_group;
		
			$get_class_group_data = unserialize($unsirData);
		
			if (is_array($get_class_group_data) && isset($get_class_group_data['subject_group']) ) {

				if(in_array($class_group, $get_class_group_data['subject_group'])){
					$filtered_all_exam_papers[] = $exam_paper;
				}
				
			}
		}
		
		wp_send_json_success( $filtered_all_exam_papers );
		
	}

	public static function get_student_record_id() {

		$examId = isset( $_POST['examId'] ) ? sanitize_text_field( $_POST['examId'] ) : 0;
		$examPaperid = isset( $_POST['exampaper_id'] ) ? sanitize_text_field( $_POST['exampaper_id'] ) : 0;
		$class_group = isset( $_POST['class_group'] ) ? sanitize_text_field( $_POST['class_group'] ) : "";
		$section_id = isset( $_POST['section_id'] ) ? sanitize_text_field( $_POST['section_id'] ) : 0;
		global $wpdb;
		$data =$wpdb->get_results( $wpdb->prepare(
			'SELECT ad.ID, ad.student_record_id 
			FROM ' . WLSM_ADMIT_CARDS . ' AS ad
			WHERE ad.exam_id = %d',
			$examId
		));
		...
		foreach($data as $key => $studentInfo){
			$table_name = WLSM_STUDENT_RECORDS ;
			$studentData =$wpdb->get_results( $var = $wpdb->prepare(
				"SELECT sd.ID, sd.name, sd.roll_number , sd.religion, sd.note, sd.optional_subject_code
				FROM {$table_name} AS sd
				WHERE sd.ID = %d AND sd.note = %s AND sd.section_id = %d ORDER BY TRIM(sd.roll_number) DESC",
				$studentInfo->student_record_id , $class_group, $section_id
			));

		
			
			if (!empty($studentData)) {
			$allStudent[$key]= $studentData;
			$blank_array[] = $var;
			// $allStudent[$key]['admit_card_id'] = $studentInfo->ID;
			}
		}
		usort($allStudent, function($a, $b) {
			return strcmp($a[0]->roll_number, $b[0]->roll_number);
		});
		...
		ob_start();
		require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/examination/add-result/studentinfo.php';
		$html = ob_get_clean();
		
		wp_send_json_success( array( 'data' => $allStudent, 'html' => $html , "blank"=> $blank_array));
	}



	public static function wlsm_add_bulk_result() {

		$page_url         = WLSM_M_Staff_Examination::get_exams_results_bulk_add();
		global $wpdb;
		$examPaperid = isset( $_POST['exam_paper_id'] ) ? absint( $_POST['exam_paper_id'] ) : 0;
		$admit_ids       = ( isset( $_POST['admit_card_id'] ) && is_array( $_POST['admit_card_id'] ) ) ? $_POST['admit_card_id'] : array();
		$obtained_marks   = ( isset( $_POST['obtained_marks'] ) && is_array( $_POST['obtained_marks'] ) ) ? $_POST['obtained_marks'] : array();

		$maximum_marks = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT maximum_marks FROM ' . WLSM_EXAM_PAPERS . ' WHERE ID = %d',
				$examPaperid
			)
		);

		if ( null === $maximum_marks ) {
			wp_send_json_error( esc_html__( 'Exam paper not found.', 'school-management' ) );
		}

		foreach ( $admit_ids as $key => $adID ) {
			if ( isset( $obtained_marks[ $key ] ) && '' !== $obtained_marks[ $key ] ) {
				$marks_obtained = WLSM_Config::sanitize_marks( $obtained_marks[ $key ] );

				if ( $maximum_marks < $marks_obtained ) {
					wp_send_json_error( esc_html__( 'Obtained Marks cannot be greater than Maximum Marks.', 'school-management' ) );
				}
			}
		}
		...
		foreach($admit_ids as $key => $adID){
			...
			$obtain_mark = $wpdb->get_row( $wpdb->prepare( 'SELECT obtained_marks , exam_paper_id FROM ' . WLSM_EXAM_RESULTS . ' WHERE exam_paper_id = %d AND admit_card_id = %d', 
			$examPaperid, $adID ));

			$exam_result_updated_data = array(
				'obtained_marks' => $obtained_marks[$key],
			);
			...
			$where_condition = array(
				'exam_paper_id' => $obtain_mark->exam_paper_id,
				'admit_card_id' => $adID
			);

			if(!empty($obtain_mark)){
				$exam_result_data['updated_at'] = current_time('Y-m-d H:i:s');
			    $newsuccess = $wpdb->update(WLSM_EXAM_RESULTS, $exam_result_updated_data, $where_condition);
			}else{
		...
		wp_send_json_success( array( 'insert' => $success, 'error'=>$error , 'update'=>$newsuccess ,  'obtained_marks' => $obtained_marks, 'page_url'=> $page_url ));
	}


	public static function get_class_based_exam() {
		$current_user = WLSM_M_Role::can( 'manage_exams' );
		$school_id = $current_user['school']['id'];
		global $wpdb;
		$class_id = isset( $_POST['class_id'] ) ? sanitize_text_field( $_POST['class_id'] ) : 0;
       ...
       $query = $wpdb->get_results( $wpdb->prepare(
		'SELECT ID, label  FROM ' . WLSM_EXAMS . '  WHERE school_id = %d AND class_id = %d', $school_id, $class_id ));


		
		wp_send_json_success(array('class_id'=> $class_id , 'query'=> $query));
	}
	...
	public static function save_exam_results() {
		...
		try {
			...
			// $exam_common_papers  = WLSM_M_Staff_Examination::get_exam_papers_by_admit_card( $school_id, $admit_card_id );
			// $exam_religion_papers  = WLSM_M_Staff_Examination::get_religion_exam_papers_by_admit_card( $school_id, $admit_card_id);
			// $exam_papers  = array_merge($exam_common_papers, $exam_religion_papers) ;
			$exam_papers  = WLSM_M_Staff_Examination::fetch_exam_papers( $school_id, $exam_id );
	...
	public static function bulk_print_result(){
		...
		$classGroup     = isset($_POST['class_group']) ? $_POST['class_group'] : "";
		...
		if (count($errors) < 1) {
			$results = WLSM_M_Staff_Examination::get_exam_results( $classGroup, $school_id, $exam_id, $section_id  );
			$exam = WLSM_M_Staff_Examination::fetch_exam( $school_id, $exam_id );
			...
			wp_send_json_success(array('html' => $html, 'json' => $json, 'results'=> $results));
		}
	...
	// excel print result 
	public static function bulk_print_result_excel(){
		$current_user = WLSM_M_Role::can('manage_exams');
		...
		$section_id  = isset($_POST['section_id']) ? absint($_POST['section_id']) : 0;
		$exam_id     = isset($_POST['exam']) ? absint($_POST['exam']) : (isset($_POST['exam_id']) ? absint($_POST['exam_id']) : 0);
		...
		try {
			...
			if ( empty( $exam_id )) {
				$errors[ 'exam_id' ] = esc_html__( 'Please select a exam.', 'school-management' );
			}
		...
		if (count($errors) < 1) {
			...
			$bulk_print = true;
			ob_start();
			require WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/print/result_print_excel.php';
			$html = ob_get_clean();
			...
			wp_send_json_success(array('html'=>$html ,'json' => $json, 'exam_id'=>$exam_id, 'section_id'=>$section_id ,  'class_group'=> $class_group));
		}

		wp_send_json_error($errors);
	}

	public static function download_exam_results_excel() {
		$current_user = WLSM_M_Role::can('manage_exams');
		...
		$class_group = isset($_REQUEST['class_group']) ? $_REQUEST['class_group'] : '';
		$section_id  = isset($_REQUEST['section_id']) ? absint($_REQUEST['section_id']) : 0;
		$exam_id     = isset($_REQUEST['exam_id']) ? absint($_REQUEST['exam_id']) : 0;

		$exam = WLSM_M_Staff_Examination::fetch_exam( $school_id, $exam_id );

		if ( empty( $exam_id ) || empty( $class_id ) || !$exam ) {
			die('Please select exam and class.');
		}
		...
		if (class_exists('\PhpOffice\PhpSpreadsheet\Reader\Html') && class_exists('\PhpOffice\PhpSpreadsheet\IOFactory')) {
			try {
				$reader = new \PhpOffice\PhpSpreadsheet\Reader\Html();
				$html_for_excel = str_replace('colspan="100"', '', $html);
				$spreadsheet = $reader->loadFromString($html_for_excel);
				...
				// Center and merge the main title across all generated data columns
	...
	public static function wlsm_get_student_religion_status(){
		$current_user = WLSM_M_Role::can('manage_exams');
	...
	$student_id    = isset($_POST['student_id']) ? absint($_POST['student_id']) : 0;
	 ...
	 $student_religion_query = $wpdb->prepare("SELECT religion  FROM {$wpdb->prefix}wlsm_student_records WHERE ID = %d", $student_id);
	 $student_religion_status = $wpdb->get_var($student_religion_query);
			
	wp_send_json_success(array('religion_status' => $student_religion_status));
