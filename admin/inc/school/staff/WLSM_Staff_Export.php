...
class WLSM_Staff_Export {
	public static function export_staff_students_table() {
		...
		try {
			...
			// Ordering by numeric roll number (1, 2, 3...) instead of string sort (1, 10, 2...).
			$query .= ' ORDER BY CAST(sr.roll_number AS UNSIGNED) ASC, sr.roll_number ASC, sr.ID ASC';
			...
			$fields = array(
				...
				esc_html__( 'Student Image', 'school-management' ),
			);

			$school_name = WLSM_M_School::get_label_text( $current_school['name'] );
			$filename    = 'students_' . date( WLSM_Config::date_format() ) . '.xlsx';
			$rows        = array();

			foreach ( $data as $row ) {
				$student_image = '';
				if ( ! empty( $row->photo_id ) ) {
					$attachment_url = wp_get_attachment_url( $row->photo_id );
					if ( $attachment_url ) {
						$student_image = sprintf( '=IMAGE("%s")', esc_url( $attachment_url ) );
					}
				}
			...
			WLSM_Export::export_students_xlsx_file( $filename, $school_name, $fields, $rows );
