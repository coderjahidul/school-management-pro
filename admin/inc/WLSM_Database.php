...
class WLSM_Database
{
	public static function activation()
	{
		...
		$sql = "CREATE TABLE IF NOT EXISTS " . WLSM_ADMINS . " (
				...
				mpo_date date NULL DEFAULT NULL,
		...
		$column_exists = $wpdb->get_var("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '" . DB_NAME . "' AND TABLE_NAME = '" . WLSM_ADMINS . "' AND COLUMN_NAME = 'blood_group'");

		// if the column doesn't exist, add it
		if (!$column_exists) {
			$wpdb->query("ALTER TABLE " . WLSM_ADMINS . " ADD blood_group varchar(5) DEFAULT NULL");
		}

		// Check if the mpo_date column already exists
		$column_exists = $wpdb->get_var("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '" . DB_NAME . "' AND TABLE_NAME = '" . WLSM_ADMINS . "' AND COLUMN_NAME = 'mpo_date'");

		// if the column doesn't exist, add it
		if (!$column_exists) {
			$wpdb->query("ALTER TABLE " . WLSM_ADMINS . " ADD mpo_date date NULL DEFAULT NULL");
		}

		// Check if the photo_id column already exists
		$column_exists = $wpdb->get_var("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '" . DB_NAME . "' AND TABLE_NAME = '" . WLSM_ADMINS . "' AND COLUMN_NAME = 'photo_id'");
		...
		$sql = "CREATE TABLE IF NOT EXISTS " . WLSM_STUDENT_RECORDS . " (
				...
				optional_subject_code varchar(40) DEFAULT NULL,
		...
		$sql = "CREATE TABLE IF NOT EXISTS " . WLSM_SUBJECTS . " (
				...
				religion varchar(40) DEFAULT NULL,
				subject_cat varchar(40) DEFAULT NULL,
				subject_group varchar(40) DEFAULT NULL,
				parent_subject varchar(40) DEFAULT NULL,
				class_school_id bigint(20) UNSIGNED DEFAULT NULL,
				created_at timestamp NULL DEFAULT CURRENT_TIMESTAMP,
				updated_at timestamp NULL DEFAULT NULL,
				PRIMARY KEY (ID),
