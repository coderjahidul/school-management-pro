...
class WLSM_Menu {
				...
				if ( $current_school['is_active'] === '1' ) {
					...
					if ( in_array( $role, array_keys( WLSM_M_Role::get_roles() ) ) ) {
						...
						if ( WLSM_M_Role::check_permission( array( 'manage_admissions', 'manage_students', 'manage_promote', 'manage_transfer_student', 'manage_certificates', 'send_notifications' ), $permissions ) ) {
							...
							$school_staff_group_student_menu = add_menu_page( esc_html__( 'Student', 'school-management' ), esc_html__( '3. Student Module', 'school-management' ), 'read', WLSM_MENU_STAFF_STUDENT, array( 'WLSM_Menu', 'school_staff_group_student' ), 'dashicons-groups', 32 );
							...
							if ( WLSM_M_Role::check_permission( array( 'manage_students' ), $permissions ) ) {
								// General - Students.
								$school_staff_students_submenu = add_submenu_page( WLSM_MENU_STAFF_STUDENT, esc_html__( 'Students', 'school-management' ), esc_html__( 'Students List', 'school-management' ), 'read', WLSM_MENU_STAFF_STUDENTS, array( 'WLSM_Menu', 'school_staff_students' ) );
							...
							if ( WLSM_M_Role::check_permission( array( 'manage_testimonials' ), $permissions ) ) {
								// School - Manage Testimonials.
								$school_staff_manage_testimonials_submenu = add_submenu_page( WLSM_MENU_STAFF_STUDENT, esc_html__( 'Testimonials', 'school-management' ), esc_html__( 'Testimonials', 'school-management' ), 'read', WLSM_MENU_STAFF_TESTIMONIALS, array( 'WLSM_Menu', 'school_staff_manage_testimonials' ) );
								add_action( 'admin_print_styles-' . $school_staff_manage_testimonials_submenu, array( 'WLSM_Menu', 'menu_page_assets' ) );
							}

							if ( WLSM_M_Role::check_permission( array( 'manage_new_testimonials' ), $permissions ) ) {
                                // School - Manage Testimonials.
                                $school_staff_new_manage_testimonials_submenu = add_submenu_page( WLSM_MENU_STAFF_STUDENT, esc_html( 'New Testimonials', 'school-management' ), esc_html( 'New Testimonials', 'school-management' ), 'read', WLSM_MENU_STAFF_NEW_TESTIMONIALS, array( 'WLSM_Menu', 'school_staff_new_manage_testimonials' ) );
                                add_action( 'admin_print_styles-' . $school_staff_new_manage_testimonials_submenu, array( 'WLSM_Menu', 'menu_page_assets' ) );
                            }
						...
						if ( WLSM_M_Role::check_permission( array( 'manage_admins', 'manage_roles', 'manage_employees', 'manage_staff_attendance', 'manage_staff_leaves' ), $permissions ) ) {
							...
							$school_staff_group_administrator_menu = add_menu_page( esc_html__( 'Administrator', 'school-management' ), esc_html__( '4. Administrator', 'school-management' ), 'read', WLSM_MENU_STAFF_ADMINISTRATOR, array( 'WLSM_Menu', 'school_staff_group_administrator' ), 'dashicons-businessman', 33 );
						...
						if (!get_option('wlsm_examination_menu')) {
							...
							if ( WLSM_M_Role::check_permission( array( 'manage_exams' ), $permissions ) ) {
								...
								$school_staff_group_examination_menu = add_menu_page( esc_html__( 'Examination', 'school-management' ), esc_html__( '6. Examination', 'school-management' ), 'read', WLSM_MENU_STAFF_EXAMINATION, array( 'WLSM_Menu', 'school_staff_group_examination' ), 'dashicons-clock', 33 );
								...
								if ( WLSM_M_Role::check_permission( array( 'manage_exams' ), $permissions ) ) {
									...
									$school_staff_exam_results_add_result_submenu = add_submenu_page( WLSM_MENU_STAFF_EXAMINATION, esc_html__( 'Add Result (New)', 'school-management' ), esc_html__( 'Add Result (New)', 'school-management' ), 'read', WLSM_MENU_STAFF_EXAM_RESULTS_ADD_RESULT, array( 'WLSM_Menu', 'school_staff_exam_results_add_result' ) );
									add_action( 'admin_print_styles-' . $school_staff_exam_results_add_result_submenu, array( 'WLSM_Menu', 'menu_page_assets' ) );

									$school_staff_exam_results_excel_print_submenu = add_submenu_page( WLSM_MENU_STAFF_EXAMINATION, esc_html__( 'Print Results Excel', 'school-management' ), esc_html__( 'Print Results Excel', 'school-management' ), 'read', WLSM_MENU_STAFF_EXAM_RESULTS_EXCEL_PRINT, array( 'WLSM_Menu', 'school_staff_exam_results_excel_print' ) );
									add_action( 'admin_print_styles-' . $school_staff_exam_results_excel_print_submenu, array( 'WLSM_Menu', 'menu_page_assets' ) );
						...
						if ( WLSM_M_Role::check_permission( array( 'manage_expenses', 'manage_income', 'manage_invoices', 'manage_fees' ), $permissions ) ) {
							...
							$school_staff_group_accounting_menu = add_menu_page( esc_html__( 'Accounting', 'school-management' ), esc_html__( '5. Account Module', 'school-management' ), 'read', WLSM_MENU_STAFF_ACCOUNTING, array( 'WLSM_Menu', 'school_staff_group_accounting' ), 'dashicons-media-spreadsheet', 33 );
						...
						if ( !get_option( 'wlsm_library_menu' ) ) {
							if ( WLSM_M_Role::check_permission( array( 'manage_library' ), $permissions ) ) {
								...
								$school_staff_group_library_menu = add_menu_page( esc_html__( 'Library', 'school-management' ), esc_html__( '7. Library', 'school-management' ), 'read', WLSM_MENU_STAFF_LIBRARY, array( 'WLSM_Menu', 'school_staff_group_library' ), 'dashicons-book', 34 );
						...
						if (!get_option('wlsm_transport_menu')) {
							...
							if ( WLSM_M_Role::check_permission( array( 'manage_transport' ), $permissions ) ) {
								...
								$school_staff_group_transport_menu = add_menu_page( esc_html__( 'Transport', 'school-management' ), esc_html__( '8. Transport', 'school-management' ), 'read', WLSM_MENU_STAFF_TRANSPORT, array( 'WLSM_Menu', 'school_staff_group_transport' ), 'dashicons-location-alt', 34 );
						...
						if (!get_option('wlsm_hostel_menu')) {
							...
							if ( WLSM_M_Role::check_permission( array( 'manage_hostel' ), $permissions ) ) {
								...
								$school_staff_group_hostel_menu = add_menu_page( esc_html__( 'SM Hostel', 'school-management' ), esc_html__( '9. Hostel', 'school-management' ), 'read', WLSM_MENU_STAFF_HOSTEL, array( 'WLSM_Menu', 'school_staff_group_hostel' ), 'dashicons-admin-home', 34 );
						...
						if (!get_option('wlsm_lessons_menu')) {
							...
							if( WLSM_M_ROLE::check_permission( array( 'manage_chapter_lessons'), $permissions)){
								$school_lectures = add_menu_page( esc_html__( 'Lessons (PI)', 'school-management' ), esc_html__( '10. Lessons (PI)', 'school-management' ), 'read', WLSM_LECTURE, array( 'WLSM_Menu', 'school_lecture' ), 'dashicons-location-alt', 34 );
								add_action( 'admin_print_styles-' . $school_lectures, array( 'WLSM_Menu', 'menu_page_assets' ) );

								$school_staff = add_submenu_page( WLSM_LECTURE, esc_html__( 'Lessons (PI)', 'school-management' ), esc_html__( 'Lessons (PI)', 'school-management' ), 'read', WLSM_LECTURE, array( 'WLSM_Menu', 'school_lecture' ) );
						...
						if(!get_option('wlsm_new_curriculum')){
							...
							if( WLSM_M_ROLE::check_permission( array( 'manage_new_curriculum'), $permissions)) {
								...
								if(WLSM_M_Role::check_permission(array('manage_new_curriculum'), $permissions)){
									// Assessment
									$student_assessment = add_submenu_page(WLSM_MENU_STAFF_NEW_CURRICULUM, esc_html__('Assessment', 'school-management'), esc_html__('1. Assessment', 'school-management'), 'read', WLSM_MENU_STAFF_STUDENT_ASSESSMENT, array('WLSM_Menu', 'school_staff_student_assessment'));
									add_action('admin_print_styles-' . $student_assessment, array('WLSM_Menu', 'menu_page_assets'));
									// Subject Based Assessment
									$Subject_based_assessment = add_submenu_page(WLSM_MENU_STAFF_NEW_CURRICULUM, esc_html__('Subject Based Assessment', 'school-management'), esc_html('2. Subject Based Assessment', 'school-management'), 'read', WLSM_MENU_STAFF_SUBJECT_BASED_ASSESSMENT, array( 'WLSM_Menu', 'school_staff_subject_based_assessment'));
	...
	// School - Manage Testimonials 
	public static function school_staff_manage_testimonials() {
		 require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/general/testimonials/route.php';
	}
	// new testimonials
    public static function school_staff_new_manage_testimonials() {
        require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/general/new-testimonials/route.php';
   }
	...
	// examination - Add result (New)
	public static function school_staff_exam_results_add_result() {
		 require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/examination/add-result/route.php';
	}

	// examination - excel print result 
	public static function school_staff_exam_results_excel_print() {
		 require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/examination/excel-print-results/route.php';
	}
	...
	public static function school_staff_student_assessment(){
		require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/staff/new-curriculum/student-assessment/route.php';
	}
	...
	public static function menu_page_assets() {
		...
		wp_enqueue_style( 'wlsm-testimonials', WLSM_PLUGIN_URL . 'assets/css/wlsm.testimonials.css', array(), time(), 'all' );
		wp_enqueue_style( 'wlsm-testimonials-two', WLSM_PLUGIN_URL . 'assets/css/wlsm.testimonials.two.css', array(), time(), 'all' );
	...
	public static function admin_enqueue_asstes(){
		wp_enqueue_style( 'custom-id-card-css', WLSM_PLUGIN_URL . 'assets/css/admin_id_new_design.css' );
	}
}
